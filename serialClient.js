...
var ModbusClient = function (socket, resHandler) {
    ...
    var api = {
		...
		// readCoilInput
		readDiscreteInputs: function (start, quantity, cb) {
			var fc = 2,
				pdu = that.pduWithTwoParameter(fc,start, quantity);
			that.makeRequest(fc, pdu, !cb?dummy:cb);
		},

		readHoldingRegisters: function (start, quantity, cb) {
			var fc = 3,
				pdu = that.pduWithTwoParameter(fc,start, quantity);
			that.makeRequest(fc, pdu, !cb?dummy:cb);
		},
		...
		writeMultipleCoils: function (address, values, cb) {
            var fc  = 15,
                pdu = that.pduWithTwoParameter(fc, address, values);

            that.makeRequest(fc, pdu, !cb?dummy:cb);
        },
		
		writeMultipleRegisters: function (address, values, cb) {
            var fc  = 16,
                pdu = that.pduWithTwoParameter(fc, address, values);
